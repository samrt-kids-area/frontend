...
import { useEditParentMutation } from "../../redux/services/apiSlice";
import toast from "react-hot-toast";
import { useEffect } from "react";

const EditParent = ({ editParentData, setEditParentData, refetch }) => {
  const [editParent, { isLoading }] = useEditParentMutation();
  ...
  const { register, handleSubmit, setValue, watch } = useForm({
    defaultValues: {
      ...
      photo: editParentData?.photo ?? null,
    },
  });

  const onSubmit = async (data) => {
    const formData = new FormData();
    if (editParentData.photo === data.photo) delete data.photo;
    Object.keys(data).forEach((key) => {
      formData.append(key, data[key]);
    });
    const res = await editParent({
      body: formData,
      id: editParentData._id,
    });
    if ("data" in res) {
      refetch();
      toast.success("Parent updated successfully");
      setEditParentData(false);
    }
    if ("error" in res) {
        ...
        <div className="relative bg-white rounded-lg shadow dark:bg-gray-700">
          ...
          <div className="flex items-center justify-between p-4 md:p-5 border-b rounded-t dark:border-gray-600">
            ...
            <button
              ...
              onClick={() => setEditParentData(!editParentData)}
            >
              <svg
                className="w-3 h-3"
                aria-hidden="true"
          ...
          <div className="p-4 md:p-5  flex flex-wrap gap-4">
            ...
            {watch("photo") && (
              <div className="w-full border h-[300px] overflow-hidden rounded-lg relative">
                <img
                  src={
                    editParentData.photo === watch("photo")
                      ? watch("photo")
          ...
          <div className="flex items-center p-4 md:p-5 border-t border-gray-200 rounded-b dark:border-gray-600">
            ...
            >
              {isLoading ? "Updateing..." : "Update Parent"}
            </button>
            <button
              onClick={() => setEditParentData(false)}
              type="button"
...
EditParent.propTypes = {
  editParentData: PropTypes.bool.isRequired,
